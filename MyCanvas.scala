...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def translate(shiftX: Double, shiftY: Double): Polygon = {
        new Polygone (cList.map((x) => (x._1 + shiftX, x._2 + shiftY)))
    }
...
case class MyCircle(val c: (Double, Double), val r: Double) extends Figure {
    ...
    override def translate(shiftX: Double, shiftY: Double): MyCircle = new MyCircle((c._1 + shiftX, c._2 + shifty), r)
...
class MyCanvas (val listOfObjects: List[Figure]) {
    ...
    def getBoundingBox: (Double, Double, Double, Double) = {
        ...
        val getShapees = listOfObjects.map((x) => x.getBoundingBox)

        xMax = cList.foldLeft(10000)((accum, x) => {
            if(x._1 > accum){
                x._1
            }
            else{
        ...
        xMin = cList.foldLeft(10000)((accum, x) => {
            if(x._1 < accum){
                x._1
            }
            else{
        ...
        yMax = cList.foldLeft(10000)((accum, y) => {
            if(y._1 > accum){
                y._1
            }
            else{
        ...
        yMin = cList.foldLeft(10000)((accum, y) => {
            if(y._1 < accum){
                y._1
            }
            else{
    ...
    def translate(shiftX: Double, shiftY: Double): MyCanvas = {
        new MyCanvas(listOfObjects.map( (valu) => {
            //Should use the previously created shift
        valu.translate(shiftX, shiftY)))
        }
    }
